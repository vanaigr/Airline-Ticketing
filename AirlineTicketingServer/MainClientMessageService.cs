...
namespace AirlineTicketingServer {
	...
	partial class MainClientMessageService : MessageService {
	...
	Either<object, LoginError> MessageService.logIn(Customer c) {
		using(var connection = new SqlConnection(Properties.Settings.Default.customersFlightsConnection)) {
	...
	Either<Seats, InputError> MessageService.seatsForFlight(int flightId) {
		using(var connection = new SqlConnection(Properties.Settings.Default.customersFlightsConnection)) {
		using(
