...
class MainOperatorMessageService : MessageService {
	...
	Either<FlightDetails, InputError> MessageService.flightDetails(int flightId) {
		...
		var scheme = BinarySeats.fromBytes(seatsSchemeBin);
		...
		for(int i = 0; i < rawPassangers.Count; i++) {
			...
			passangers.Add(new BookedPassanger{
				...
				document = BinaryDocument.fromBytes(it.documentBin)
			});
			arrivalStatus.Add(it.arrived);
		}

		for(int i = 0; i < classes.Length; i++) {
			Debug.Assert(rawSeatsClasses[i].index == i);
		...
		Either<Success, InputError> MessageService.updateArrivaltatus(int flightId, Dictionary<int, bool> seatArrived) {
			using(var connection = new SqlConnection(Properties.Settings.Default.customersFlightsConnection)) {
			using(
			var command = new SqlCommand(
				@"
